...
import { getCharacter, searchCharacters, debounce } from './index';
...
it('calls get character with the correct url', () => {
...
it('calls search characters with the correct url', () => {
    global.fetch = jest.fn().mockImplementation(() => Promise.resolve({ json: () => {} }));
    searchCharacters('Rick and Morty');
    expect(global.fetch).toBeCalled();
    expect(global.fetch).toBeCalledWith(`${apiRoute}/character/?name=${encodeURI('Rick and Morty')}`);
});
